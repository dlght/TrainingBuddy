...
import type { ExerciseWeightPoint } from "@/models/session";
...
export function WeightTrendChart({ points }: WeightTrendChartProps) {
  return (
    <View style={styles.section}>
      ...
      {points.length === 0 ? (
        <Text style={styles.emptyText}>Log completed sets to build a weight trend.</Text>
      ) : (
        <LineChart values={points.map((point) => point.weight)} suffix="" />
      )}
    </View>
  );
}

function LineChart({ values, suffix }: { values: number[]; suffix: string }) {
  const width = 280;
  const height = 132;
  const padding = 20;
